...
def callback(data):
    ...
    if exp_num == 5 or exp_num == 6:
      ...
      while True:
        ...
        current_pan = abs(get_pan_position())
        ...
        f = np.float32([[-5.0 - weight, 1621.0 + weight, 1.0]])
        f = f.T
        f = np.dot(h, f)
        f = f / f[2][0]
        ...
        s = np.float32([[5.0 + weight, 1621.0 + weight, 1.0]])
        s = s.T
        s = np.dot(h, s)
        s = s / s[2][0]
        ...
        t = np.float32([[-5.0 - weight, 1611.0 - weight, 1.0]])
        t = t.T
        t = np.dot(h, t)
        t = t / t[2][0]
        ...
        fo = np.float32([[5.0 + weight, 1611.0 - weight, 1.0]])
        fo = fo.T
        fo = np.dot(h, fo)
        fo = fo / fo[2][0]
...
def exp_miki_img():
    rospy.init_node('exp_miki_img_56', anonymous=True)
    #rospy.set_param("exp_miki_img/switch", 1)
